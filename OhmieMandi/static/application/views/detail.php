<div class="main-content" id="detail">
	<div class="row">
		<div id="detail-gallery" class="large-6 columns">		
			<img src="<?php echo base_url(); ?>public/images/sh-thumb-210x210.png" alt="P-38 versus Zeros" />
			<img src="<?php echo base_url(); ?>public/images/sh-thumb2-210x210.png" alt="P-38 takes flak against a mini-boss" />
			<img src="<?php echo base_url(); ?>public/images/sh-thumb-440x210.png" alt="P-38 versus giant airship" />
		</div>
		<div id="detail-copy" class="large-6 columns">
			<h1>Steel Hawks</h1>
			<p>Your base was attacked and now it's time for you to return the fight! Shoot your way through 30 waves of gunships, bombers, and fighters. Power up your guns for an even deadlier attack! Intense dogfights await as you strap into your cockpit and fire up your engines. Good hunting, pilot!</p>
			<ul>
				<li>Requires 186MB of free space</li>
				<li>Compatibility: Mac OSX 10.6 or higher</li>
			</ul>
			<p class="price">$3.99</p>
			<?php echo
			"<a href=?action=checkout class='button'>Download</a>"
			?>
		</div>
		
	</div>
</div>
	